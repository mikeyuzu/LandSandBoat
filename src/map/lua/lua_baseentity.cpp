...
uint16 CLuaBaseEntity::getMissionStatusLower(MissionLog logId) const
{
    if (m_PBaseEntity->objtype != TYPE_PC)
    {
        ShowWarning("Invalid entity type calling function (%s).", m_PBaseEntity->getName());
    ...
    {
        auto* PChar = static_cast<CCharEntity*>(m_PBaseEntity);
        statusLower = PChar->m_missionLog[static_cast<uint8_t>(logId)].statusLower;
    }
    else
    {
        ShowError("Lua::getMissionStatusLower: missionLogID %i is invalid", static_cast<uint8_t>(logId));
    }
