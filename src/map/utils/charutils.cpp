...
auto AddItem(CCharEntity* PChar, uint8 LocationID, std::unique_ptr<CItem> PItem, bool silence) -> uint8
{
    ...
    {
        // Pitemを複製する
        auto copyItem = std::make_unique<CItem>(*PItem);

        AddCustomItemBook(PChar, LocationID, std::move(copyItem), silence);
    }

    if (LocationID == LOC_INVENTORY && settings::get<bool>("map.CUSTOM_INVENTORY"))
    {
        return AddItemCustom(PChar, LocationID, std::move(PItem), silence);
    }

    return AddItemInventory(PChar, LocationID, std::move(PItem), silence);
}

auto AddItemInventory(CCharEntity* PChar, uint8 LocationID, std::unique_ptr<CItem> PItem, bool silence) -> uint8
{
    ...
    auto copyItem = std::make_unique<CItem>(*PItem);
    uint8 SlotID   = PStorage->InsertItem(std::move(copyItem));
...
/// <returns></returns>
auto AddItemCustom(CCharEntity* PChar, uint8 LocationID, std::unique_ptr<CItem> PItem, bool silence) -> uint8
{
    {
        ...
        if (itemId >= 4096 && itemId <= 4103)
        {
            Ingredient_item = itemId;
        }
...
/// <param name="silence"></param>
/// <returns></returns>
void AddCustomItemBook(CCharEntity* PChar, uint8 LocationID, std::unique_ptr<CItem> PItem, bool silence)
{
            ...
            if (PChar->jobs.job[sjob] < PChar->jobs.genkai || (PChar->jobs.job[sjob] >= PChar->jobs.genkai && PChar->jobs.exp[sjob] < GetExpNEXTLevel(PChar->jobs.job[sjob] - 1)))
            {
                ...
                if (PChar->jobs.exp[sjob] >= GetExpNEXTLevel(PChar->jobs.job[sjob]))
                {
                    ...
                    else
                    {
                        ...
                        PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF, std::make_unique<GP_SERV_COMMAND_BATTLE_MESSAGE2>(PChar, PMob, PChar->jobs.job[sjob], 0, MsgBasic::LevelUp));
                        PChar->pushPacket<CCharStatusPacket>(PChar);
    ...
    if (PWeapon && PWeapon->isUnlockable() && !PWeapon->isUnlocked())
    {
        ...
        if (settings::get<bool>("map.ENABLE_TRIAL_WS_POINT_MESSAGE"))
        {
            ...
            if (trial_ws_points >= final_ws_points)
            {
                ...
                PChar->pushPacket<GP_SERV_COMMAND_CHAT_STD>(PChar, CHAT_MESSAGE_TYPE::MESSAGE_SYSTEM_1, msg.c_str(), "");
            }
            else
            {
