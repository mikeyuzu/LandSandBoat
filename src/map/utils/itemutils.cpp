        ...
        FOR_DB_MULTIPLE_RESULTS(rset)
        {
            ...
            if (PItem != nullptr)
            {
                ...
                if (PItem->isType(ITEM_WEAPON))
                {
                    ...
                    int unclock_point = rset->get<uint16>("unlock_points");
                    if (unclock_point == 300)
                    {
                        static_cast<CItemWeapon*>(PItem)->setTotalUnlockPointsNeeded(settings::get<int>("map.TRIAL_WS_POINTS"));
                    }
                    else if (unclock_point == 500)
                    {
                        static_cast<CItemWeapon*>(PItem)->setTotalUnlockPointsNeeded(settings::get<int>("map.RETRIAL_WS_POINTS"));
                    }
                    else
                    {
