...
xi.settings.main =
{
    ...
    SERVER_MESSAGE =
        "プロジェクトの最新情報については、https://github.com/LandSandBoat/server をご覧ください。\n" ..
        "遊んでいただきありがとうございます。砂漠の航海をお楽しみください！",
    ...
    ENABLE_COP       = 1,  -- ジラートの幻影
    ENABLE_TOAU      = 1,  -- プロマシアの呪縛
    ENABLE_WOTG      = 1,  -- アトルガンの秘宝
    ENABLE_ACP       = 1,  -- アルタナの神兵
    ENABLE_AMK       = 1,  -- 追加シナリオ「シャントット帝国の陰謀」
    ENABLE_ASA       = 1,  -- 追加シナリオ「アシュタリフの双蛇」
    ENABLE_ABYSSEA   = 1,  -- アビセア
    ENABLE_SOA       = 1,  -- 追加シナリオ「 seekers of adoulin 」
    ENABLE_ROV       = 1,  -- 追加シナリオ「 rise of the zilart 」
    ENABLE_TVR       = 1,  -- 追加シナリオ「 treasures of aht urhgan 」
    ...
    REGIME_WAIT           = 1, -- リテール版と同様に、ゲーム内時間0:00まで待機させる。0の場合は待機時間なし。
    FOV_REWARD_ALLIANCE   = 0, -- アライアンスに所属している間もフィールド・オブ・ヴァラーを許可する。(リテール版のデフォルト動作: 0)
    GOV_REWARD_ALLIANCE   = 1, -- アライアンスに所属している間もグラウンド・オブ・ヴァラーを許可する。(リテール版のデフォルト動作: 1)
    ...
    CAP_CURRENCY_SPARKS    = 99999, -- 功績ポイントの上限
    ...
    TRUST_ALTER_EGO_EXTRAVAGANZA_MESSAGE =
        "\n \n" .. -- これらの改行の間のスペースは意図的なものです
        "\129\153\129\154 フェイス「モグガーデン開拓キャンペーン」開催中！ \129\154\129\153\n" .. -- メッセージ修正
        "今がフェイスの仲間を増やす絶好の機会です！", -- メッセージ修正

    TRUST_ALTER_EGO_EXPO_MESSAGE =
        "\n \n" .. -- これらの改行の間のスペースは意図的なものです
        "\129\153\129\154 フェイス「エキスポ」開催中！ \129\154\129\153\n" ..
        "フェイスのHP、MP、状態異常耐性が向上します！",
